...
*   **PDF 智能转 Markdown：** 支持将 PDF 自动按页转换为 Markdown（如 `xxx_1.md` 到 `xxx_N.md`），并通过本地多模态模型（如 `qwen3-vl:8b/30b`）识别公式与附图说明。
...
2.  第一次运行脚本时，它将先把 `data/` 目录中的 PDF **按页**转换为 Markdown（例如 `某文档_1.md` 到 `某文档_N.md`，公式输出为 LaTeX，附图转为简要描述），再统一处理 Markdown 文档，构建向量索引和知识图谱，并将它们保存到 `model_files/` 目录。这可能需要一些时间，具体取决于您的文档大小和硬件。
