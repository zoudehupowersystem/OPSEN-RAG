...
*   **Local-first PDF 解析：** 新增 `pdf_conversion.mode = auto | local | llm`，优先使用 PyMuPDF 做本地结构化抽取，仅在复杂页或扫描页时回退到多模态模型。
*   **结构化页级元数据：** 可为每个分页 Markdown 同步导出 `*.json`，保留页码、标题层级、bbox、抽取模式等信息，便于后续精确引用与前端高亮。
*   **证据可追溯检索：** 检索结果与答案生成链路保留来源、页码、标题、位置框等元数据，并支持邻居段扩展与向量/图谱融合重排。
