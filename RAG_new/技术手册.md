...
3. 使用本地模型（Ollama）完成：
   - PDF 多模态识别（默认 `qwen3-vl:30b`）
   - 实体关系抽取与答案生成（默认 `qwen3-vl:30b`）

> 上述默认值由 `config/rag_config.json` 管理，可按环境直接调整。
...
系统新增 `config/rag_config.json`，用于集中管理提示词和模型配置，避免将提示词/模型名硬编码在源码中。

主要字段：

- `models`：`embedding` / `pdf_vision` / `entity_extraction` / `answer_generation`
- `prompts`：`pdf_conversion` / `entity_extraction` / `answer_generation`
- `pdf_conversion`：PDF 分页转换参数（DPI、重试、上下文长度等）
- `ollama_options`：实体抽取与答案生成的采样参数

代码行为：

- `graph_rag.py` 内置默认配置。
- 启动时会读取 `config/rag_config.json` 并与默认值深度合并。
- 若配置文件缺失，系统使用内置默认值继续运行。

---

## 2.2 新增设计要点（参考 OpenDataLoader）

本轮改造引入了三个核心原则：

1. **Local-first**：优先使用确定性本地版面抽取，而不是所有页面都走多模态模型。
2. **Structured output**：每页除 Markdown 外，还输出带 `bbox / type / level / page_number` 的 JSON 元数据。
3. **Traceable retrieval**：chunk、检索结果和最终答案均尽量保留来源、页码、标题和坐标信息。

这样做的直接收益是：可解释性更强、重现性更高、普通可选文本 PDF 处理速度明显更快，同时保留在扫描页/复杂页上回退到视觉模型的能力。
...
- 额外生成同名结构化侧车文件：`<pdf_stem>_1.json ... <pdf_stem>_N.json`。
- 增量跳过：若分页 md / json 均存在且更新时间不早于 pdf，则跳过。
- 异常时输出类型、堆栈与排查建议。

---

## 4. PDF 解析模块（`PDFToMarkdownConverter`）

当前版本不再局限于“多模态逐页 OCR”，而是支持三种模式：

- `local`：PyMuPDF 本地结构化抽取
- `llm`：逐页多模态识别
- `auto`：先做本地质量评估，再按页选择 local / llm

其中 `auto` 模式会参考：文字密度、文本块数量、字符清洁度（是否存在较多乱码/异常字形）等指标。
...
5. 对长页默认使用更高上下文与输出上限（`num_ctx=16384`，`num_predict=4096`）。
...
- 识别 `503/timeout/temporarily/返回空内容` 为可重试错误。
- 默认重试 5 次，退避等待（`retry_wait_s * attempt`）。
...
`process_documents(show_entity_relations=False)`：
...
> 当 `show_entity_relations=True`（首次构建建议开启）时，会打印每个 chunk 的实体与关系抽取结果。

### 7.4 检索融合

- 新增前后段扩展：对命中 chunk 自动拼接前后 1 段（前文段/当前段/后文段），减少断句导致的证据缺失。
- 回答阶段要求优先引用检索证据，并对结论标注证据编号。

`hybrid_search(query)`：

1. 向量阶段先高召回（candidate_k），再做筛选。
2. 在向量分数基础上叠加关键词重排分数（query/content 分词重合度）。
3. 图扩展阶段按 `entity -> chunk` 的反向边（`in_edges(chunk)`）回溯实体。
4. 进行融合排序，返回最终上下文。

> 说明：此策略可显著改善“语义相近但问题关键词不匹配”导致的误召回。
...
## 9. 模型配置建议（通过 `config/rag_config.json`）

### 9.1 默认模型

- PDF 多模态识别：`models.pdf_vision`（默认 `qwen3-vl:30b`）
- 实体关系抽取：`models.entity_extraction`（默认 `qwen3-vl:30b`）
- 答案生成：`models.answer_generation`（默认 `qwen3-vl:30b`）
...
- `UNEXPECTED`：通常为底层权重兼容提示，不是致命错误。
- `跳过 PDF 转换（分页 Markdown 已是最新）`：说明增量策略命中。
- `模型已从本地加载`：向量索引和图谱复用成功。

### 10.2 检索结果质量控制

当前检索会做以下质量控制以减少低相关噪声：

1. 向量高召回后重排（关键词覆盖率融合）。
2. 命中段自动拼接前后段，增强上下文完整性。
3. 图谱结果仅保留与 query 有关键词重合的实体/关系。
4. 输出前进行空内容过滤、非有限分数过滤和内容去重。
